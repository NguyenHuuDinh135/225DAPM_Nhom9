...
public record SendIncidentFeedbackCommand : IRequest<Result>
{
...
public class SendIncidentFeedbackCommandHandler : IRequestHandler<SendIncidentFeedbackCommand, Result>
{
    ...
    public async Task<Result> Handle(SendIncidentFeedbackCommand request, CancellationToken cancellationToken)
    {
        var incident = await _context.TreeIncidents
            .FindAsync([request.IncidentId], cancellationToken);

        if (incident is null)
            return Result.Failure($"TreeIncident {request.IncidentId} not found.");
