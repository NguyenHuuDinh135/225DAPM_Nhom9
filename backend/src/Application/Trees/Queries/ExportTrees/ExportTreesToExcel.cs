...
public class ExportTreesToExcelQueryHandler : IRequestHandler<ExportTreesToExcelQuery, byte[]>
{
    ...
    public async Task<byte[]> Handle(ExportTreesToExcelQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Lọc theo tình trạng (chỉ khi không có TreeIds)
            if (!string.IsNullOrWhiteSpace(request.Condition) && (request.TreeIds == null || !request.TreeIds.Any()))
            {
                query = query.Where(t => t.Condition != null && t.Condition == request.Condition);
            }
            
            // Lọc theo từ khóa tìm kiếm (chỉ khi không có TreeIds)
            if (!string.IsNullOrWhiteSpace(request.SearchTerm) && (request.TreeIds == null || !request.TreeIds.Any()))
            {
        ...
        {
            // Log the error
            Console.WriteLine($"Error exporting trees: {ex.Message}");
