...
export function LoginForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.error((err as ApiError).message ?? "Đăng nhập thất bại");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card className="overflow-hidden p-0 border-none shadow-2xl shadow-slate-200/60 rounded-3xl">
        <CardContent className="p-0">
          <form className="p-8 md:p-10" onSubmit={handleSubmit}>
            <FieldGroup>
              <div className="flex flex-col items-center gap-2 text-center mb-4">
                <h1 className="text-2xl font-black tracking-tight text-foreground">Chào mừng trở lại</h1>
              ...
              <Field>
                ...
                <Input id="email" type="email" placeholder="email@danangxanh.vn" required className="h-12 rounded-xl" />
              </Field>
              <Field>
                <div className="flex items-center">
                  <FieldLabel htmlFor="password">Mật khẩu</FieldLabel>
                  <a href="#" className="ms-auto text-sm underline-offset-2 hover:underline text-muted-foreground">
                    Quên mật khẩu?
                  </a>
                </div>
