...
export function CreateWorkDialog({ onCreated }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/api/planning`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then((r) => r.ok ? r.json() : [])
      .then((data: Plan[]) => setPlans(data))
      .catch(() => {})
    fetch(`${BASE_URL}/api/lookups/work-types`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then((r) => r.ok ? r.json() : [])
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!workTypeId || !planId) { toast.error("Vui lòng điền đầy đủ thông tin"); return }
    const token = localStorage.getItem("access_token")
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/work-items`, {
        ...
        body: JSON.stringify({ workTypeId: Number(workTypeId), planId: Number(planId), creatorId, startDate: startDate || null, endDate: endDate || null }),
      })
      if (!res.ok) throw new Error()
      const workType = workTypes.find((w) => w.id === Number(workTypeId))
      const plan = plans.find((p) => p.id === Number(planId))!
      onCreated({ id: Date.now(), workTypeId: Number(workTypeId), workTypeName: workType?.name ?? null, planId: Number(planId), planName: plan.name, creatorId, startDate: startDate || null, endDate: endDate || null, status: "New", statusName: "New" })
      toast.success("Đã tạo công tác mới")
      setOpen(false)
      setWorkTypeId(""); setPlanId(""); setStartDate(""); setEndDate("")
    } catch {
      toast.error("Tạo công tác thất bại")
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-background rounded-2xl border shadow-xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-5 border-b">
          <div>
            <h2 className="font-semibold text-xl">Tạo công tác mới</h2>
            <p className="text-sm text-muted-foreground mt-0.5">Điền thông tin để tạo công tác</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <Label htmlFor="workType" className="text-sm font-medium">Loại công tác</Label>
              <Select value={workTypeId} onValueChange={setWorkTypeId}>
                ...
                <SelectContent>
                  {workTypes.map((w) => <SelectItem key={w.id} value={String(w.id)}>{w.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="flex flex-col gap-2">
